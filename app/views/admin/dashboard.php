...
<head>
    ...
    <style>
        ...
        .btn-sm-primary { background: linear-gradient(135deg, #09637E, #088395); color: white; border: none; padding: 0.5rem 1rem; border-radius: 12px; cursor: pointer; font-size: 0.75rem; font-weight: 500; transition: 0.2s; }
        .btn-sm-primary:hover { transform: translateY(-1px); box-shadow: 0 2px 8px rgba(8,131,149,0.3); }
        .siswa-info { display: flex; align-items: center; gap: 10px; }
        .siswa-avatar-sm { width: 32px; height: 32px; border-radius: 50%; background: #EBF4F6; display: flex; align-items: center; justify-content: center; overflow: hidden; }
        .siswa-avatar-sm i { font-size: 1.2rem; color: #09637E; }
        .siswa-avatar-sm img { width: 100%; height: 100%; object-fit: cover; }
        .siswa-name { font-weight: 500; color: #1e293b; }
        .time-cell { font-family: monospace; font-size: 0.85rem; }
        .time-badge { background: #EBF4F6; padding: 0.3rem 0.8rem; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px; font-size: 0.75rem; color: #09637E; }
        .time-badge-out { background: #E0F2FE; padding: 0.3rem 0.8rem; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px; font-size: 0.75rem; color: #0284C7; }
        .status-belum-pulang { background: #FEF3C7; color: #D97706; padding: 0.3rem 0.8rem; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px; font-size: 0.7rem; font-weight: 500; }
        .status-hadir-badge { background: #d1fae5; color: #065f46; padding: 0.3rem 0.8rem; border-radius: 20px; display: inline-flex; align-items: center; gap: 6px; font-size: 0.7rem; font-weight: 500; }
        ...
        <?php if ($tab == 'dashboard'): ?>
            <div class="card">
                <div class="card-header">
                    <h3><i class="fas fa-users"></i> Data Siswa Terbaru</h3>
                    <a href="?tab=siswa" class="btn-add">Lihat Semua</a>
                </div>
                <div class="table-responsive">
                    <table class="data-table">
                        <thead>
                            <tr><th>NIS</th><th>Nama</th><th>Kelas</th><th>Orang Tua</th></tr>
                        </thead>
                        <tbody>
                            <?php foreach (array_slice($dataSiswa, 0, 5) as $s): ?>
                            <tr>
        ...
        <?php elseif ($tab == 'siswa'): ?>
            <div class="card">
                ...
                <div class="table-responsive">
                    <table class="data-table">
                        <thead>
                            <tr><th>Foto</th><th>NIS</th><th>Nama</th><th>Kelas</th><th>Orang Tua</th><th>Tag ID</th><th>Aksi</th></tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <div class="avatar-sm">
                                        <?php
                                        $files = glob(UPLOAD_PATH_SISWA . 'siswa_' . $s['student_id'] . '_*');
                                        if (!empty($files)) {
                                            $filename = basename($files[0]);
                                            $photoUrl = APP_URL . '/public/uploads/siswa/' . $filename;
                                            echo '<img src="' . $photoUrl . '" class="avatar-img" style="width:40px;height:40px;border-radius:50%;object-fit:cover;">';
                                        } else {
                                            echo '<i class="fas fa-user-circle fa-2x" style="color:#94a3b8;"></i>';
                                        }
                                        ?>
                                    </div>
                                </td>
                                ...
                                <td>
                                    <div class="action-buttons">
                                        <button class="btn-edit" onclick="editSiswa(<?= $s['student_id'] ?>)"><i class="fas fa-edit"></i> Edit</button>
                                        <button class="btn-delete" onclick="hapusSiswa(<?= $s['student_id'] ?>)"><i class="fas fa-trash"></i> Hapus</button>
                                        <button class="btn-upload" onclick="uploadFotoSiswa(<?= $s['student_id'] ?>)"><i class="fas fa-camera"></i> Foto</button>
                                    </div>
                                </td>
                            </tr>
                            <?php endforeach; ?>
        ...
        <?php elseif ($tab == 'guru'): ?>
            <div class="card">
                <div class="card-header">
                    ...
                    <button class="btn-add" onclick="showTambahGuru()"><i class="fas fa-plus"></i> Tambah Guru</button>
                </div>
                        ...
                        <div class="guru-premium-card">
                            <div class="guru-premium-header">
                                <div class="guru-premium-avatar">
                                    <?php
                                    $files = glob(UPLOAD_PATH_GURU . 'guru_' . $g['teacher_id'] . '_*');
                                    if (!empty($files)) {
                                        $filename = basename($files[0]);
                                        $photoUrl = APP_URL . '/public/uploads/guru/' . $filename;
                                        echo '<img src="' . $photoUrl . '" class="avatar-img" style="width:70px;height:70px;border-radius:50%;object-fit:cover;">';
                                    } else {
                            ...
                            <div class="guru-premium-footer">
                                <button class="btn-edit" onclick="editGuru(<?= $g['teacher_id'] ?>)"><i class="fas fa-edit"></i> Edit</button>
                                <button class="btn-delete" onclick="hapusGuru(<?= $g['teacher_id'] ?>)"><i class="fas fa-trash"></i> Hapus</button>
                                <button class="btn-upload" onclick="uploadFotoGuru(<?= $g['teacher_id'] ?>)"><i class="fas fa-camera"></i> Foto</button>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
